...
ラウンド数はタスクファイルの Decision Log 内の `[team-review] POST` エントリ数で判定する。

### 5-3. 修正タスクの作成

Medium+ の発見事項を修正タスクリストに変換し、タスクファイル（`.claude/docs/decisions/task-{id}-{feature}.md`）の `## Fix Tasks` セクションに保存する（セクション内容を上書き）：
