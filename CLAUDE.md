...
| Tier | /startproject | /team-implement | /team-review | Gemini | Codex |
|------|-------------|----------------|-------------|--------|-------|
| **XS** | 不要 | Claude 直接 | スキップ | 不要 | 不要 |
| **S** | Phase 1 のみ | Claude 直接 | Claude 直接 | 不要 | 不要 |
| **M** | Phase 1 + Codex サブエージェント | Claude or 小チーム | 2 レビュアー | 必要時のみ | サブエージェント |
...
3. **M**: Codex サブエージェントで設計 / **L**: Agent Teams で調査・設計
