...
| `/fs-ops` | mkdir、rm、cp、mv、chmod、ln、touch（影響範囲チェック付き） |
| `/deploy` | **全 git 操作**（commit、push、PR作成、log、diff、branch 等）、Linear MCP |
...
- **Deploy Workflow モード**: PR 作成 + push + Linear 投稿（フルデプロイフロー）
- **Ad-hoc Git モード**: 単発の git 操作（commit、log、diff、branch 等）

Ad-hoc Git モードでは **Push/Pull 分類** により操作を制御する：
- **Push-type（書き込み）**: `git commit`, `git push`, `git merge`, `git rebase` 等 → main 上では feature ブランチを自動作成
- **Pull-type（読み取り）**: `git log`, `git diff`, `git show`, `git blame` 等 → ブランチ制限なし

| 操作 | ルーティング |
|------|------------|
| `git status`, `git branch --show-current`, `git rev-parse`, `git config --get` | Claude が直接実行OK |
| その他の全 git 操作（commit、push、log、diff、branch、blame 等） | `/deploy` スキル経由 |
| `.gitignore` 等の設定ファイル読み取り | Read ツール経由で直接OK |
| 特定ファイルの読み取り・特定シンボルの検索 | Read / Grep / Glob ツール経由で直接OK |
| docker/ruff/uv 操作（アドホック） | サブエージェント経由 |
...
/team-implement            Claude 直接 or Agent Teams（tier に応じて適応）
                           Linear: "In Progress" に変更
    ↓ 完了後
/team-review               Claude 直接 or 並列レビュー（tier に応じて適応）
                           Linear: "In Progress" に変更
    ↓ レビュー完了後（自動修正ループ含む）
/deploy                    PR 作成 + push
                           Linear: "In Review" に変更
```

1. Claude がタスクサイズを判定（XS/S/M/L）→ ワークフローを適応
2. Claude がユーザーと要件ヒアリング、コードベースを直接分析
3. **M**: OpenCode サブエージェントで設計 / **L**: Agent Teams で調査・設計
4. 承認後、`/team-implement` で実装（tier に応じた規模）— Linear を "In Progress" に
5. `/team-review` でレビュー（tier に応じた規模、Medium+ は自動修正ループ）— Linear を "In Progress" に
6. `/deploy` で PR 作成・push（自動発火）— Linear を "In Review" に
...
`UserPromptSubmit` hook (`agent-router.py`) がプロンプトを分析し、5つのスキルのいずれかを `additionalContext` で提案する。
...
| ファイルシステム操作 | `/fs-ops` | 「ディレクトリを作って」「ファイルを削除して」「移動して」 |
| PR作成・push・全 git 操作 | `/deploy` | 「PRを作って」「デプロイして」「コミットして」「差分を見せて」 |

**自動ルーティングが発火しないケース（例外的な処理）:**
- 質問・説明依頼（「これは何？」「なぜ失敗した？」）
- 単発の軽微な操作（「修正して」「フォーマットして」）
- 短いプロンプト（5文字未満）
- 既にスキル名が明示されている場合

> **Note**: git 操作（「コミットして」「差分を見せて」等）は軽微な操作に含まれない。`/deploy` スキルにルーティングされる。
>
> **Note**: ファイルシステム操作（「削除して」「移動して」等）は軽微な操作に含まれない。`/fs-ops` スキルにルーティングされる。
